...
public class RegionLocationFinder {
  ...
  private boolean scheduleFullRefresh() {
    ...
    Map<ServerName, List<HRegionInfo>> assignments = am.getAssignments();
    if (assignments == null) {
      return false;
    }

    boolean includesUserTables = false;
    for (Entry<ServerName, List<HRegionInfo>> e : assignments.entrySet()) {
      for (HRegionInfo hri : e.getValue()) {
        cache.refresh(hri);
        includesUserTables = includesUserTables || !hri.isMetaTable();
      }
    }
  ...
  protected HDFSBlocksDistribution internalGetTopBlockLocation(HRegionInfo region) {
    try {
      HTableDescriptor tableDescriptor = getTableDescriptor(region.getTableNameAsString());
      if (tableDescriptor != null) {
        HDFSBlocksDistribution blocksDistribution = HRegion.computeHDFSBlocksDistribution(
          getConf(), tableDescriptor, region.getEncodedName());
   ...
   * @throws IOException
   */
  protected HTableDescriptor getTableDescriptor(String tableName) throws IOException {
  ...
  protected List<ServerName> mapHostNameToServerName(List<String> hosts) {
    ...
    for (ServerName sn : regionServers) {
      if (!hostToServerName.containsKey(sn.getHostname())) {
        hostToServerName.put(sn.getHostname(), new ArrayList<ServerName>());
      }
      hostToServerName.get(sn.getHostname()).add(sn);
    }
  ...
  @VisibleForTesting
  protected long getLastFullRefresh() {
