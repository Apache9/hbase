...
public class KeyValue implements Writable, HeapSize {
  ...
  public static class KVComparator implements java.util.Comparator<KeyValue> {
    ...
    public int compareKey(byte[] key, int koff, int klen, byte[] row, int roff, short rlen,
        byte[] fam, int foff, byte flen, byte[] col, int coff, int clen, long ts, byte type) {
  ...
  public static class MetaKeyComparator extends KeyComparator {
    public int compareRows(byte [] left, int loffset, int llength,
      ...
      int rightFarDelimiter = getDelimiterInReverse(right, rightDelimiter, 
          rlength - (rightDelimiter - roffset), HRegionInfo.DELIMITER);
      ...
      if (result != 0) {
        ...
        if (leftFarDelimiter < 0 && rightFarDelimiter >= 0) {
          // Nothing between .META. and regionid.  Its first key.
          return -1;
        } else if (rightFarDelimiter < 0 && leftDelimiter >= 0) {
          return 1;
        } else if (leftDelimiter < 0 && rightDelimiter < 0) {
          return 0;
        }
    ...
    /**
     * @see {@link KeyComparator#getFakedKey(byte[] leftKey, byte[] rightKey)}
     */
  ...
  public static class KeyComparator
    ...
    /**
     * Generate a faked byte array if possible, see HBASE-7845
     * we need to ensure: leftKey < newKey <= rightKey
     * @return newKey: the new generated faked key
     */
    public byte[] getFakedKey(final byte[] leftKey, final byte[] rightKey) {
      if (rightKey == null) {
