...
public class HLogSplitterHandler extends EventHandler {
  ...
  public void process() throws IOException {
    ...
    try {
      ...
      switch (status) {
      case DONE:
        endTask(zkw, TaskState.TASK_DONE.get(serverName),
          tot_wkr_task_done, curTask, curTaskZKVersion.intValue());
        break;
      case PREEMPTED:
      ...
      case ERR:
        if (server != null && !server.isStopped()) {
          endTask(zkw, TaskState.TASK_ERR.get(serverName),
            tot_wkr_task_err, curTask, curTaskZKVersion.intValue());
          break;
        }
      ...
      case RESIGNED:
        ...
        endTask(zkw, TaskState.TASK_RESIGNED.get(serverName),
          tot_wkr_task_resigned, curTask, curTaskZKVersion.intValue());
        break;
      }
    } finally {
   ...
   * @param ctr
   */
  public static void endTask(ZooKeeperWatcher zkw, byte[] data, AtomicLong ctr, String task,
      int taskZKVersion) {
    try {
      if (ZKUtil.setData(zkw, task, data, taskZKVersion)) {
        LOG.info("successfully transitioned task " + task + " to final state " + Bytes.toString(data));
        ctr.incrementAndGet();
        return;
      }
      LOG.warn("failed to transistion task " + task + " to end state " + Bytes.toString(data)
          + " because of version mismatch ");
    } catch (KeeperException.BadVersionException bve) {
      LOG.warn("transisition task " + task + " to " + Bytes.toString(data)
          + " failed because of version mismatch", bve);
    } catch (KeeperException.NoNodeException e) {
      LOG.fatal(
        "logic error - end task " + task + " " + Bytes.toString(data)
          + " failed because task doesn't exist", e);
    } catch (KeeperException e) {
      LOG.warn("failed to end task, " + task + " " + Bytes.toString(data), e);
    }
