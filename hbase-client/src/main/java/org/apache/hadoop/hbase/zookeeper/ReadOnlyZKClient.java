  ...
  private abstract class ZKTask<T> extends Task {
        ...
        public void exec(ZooKeeper alwaysNull) {
          ...
          if (code == Code.OK) {
            ...
            if (code == Code.SESSIONEXPIRED) {
              LOG.warn("Session expired, close and reconnect");
              try {
                zk.close();
            ...
            if (ZKTask.this.delay(retryIntervalMs, maxRetries)) {
              LOG.warn("Failed for " + operationType + " of " + path + ", code = " + code +
                  ", retries = " + ZKTask.this.retries);
              tasks.add(ZKTask.this);
            } else {
              LOG.warn("Failed for " + operationType + " of " + path + ", code = " + code +
                  ", retries = " + ZKTask.this.retries + ", give up");
              future.completeExceptionally(KeeperException.create(code, path));
            }
          }
        }
    ...
    public void connectFailed(IOException e) {
      if (delay(retryIntervalMs, maxRetries)) {
        LOG.warn(
          "Failed to connect to zk for " + operationType + " of " + path + ", retries = " + retries,
          e);
        tasks.add(this);
      } else {
        LOG.warn("Failed to connect to zk for " + operationType + " of " + path + ", retries = " +
            retries + ", give up",
          e);
