...
public class ScannerCallable extends RegionServerCallable<Result[]> {
  ...
  private ScanResponse next() throws IOException {
    ...
    incRPCcallsMetrics();
    ScanRequest request =
        RequestConverter.buildScanRequest(scannerId, caching, false, nextCallSeq, scan.getLimit());
    try {
  ...
  @Override
  public Result[] rpcCall() throws IOException {
    ...
    PayloadCarryingRpcController controller = getController();
