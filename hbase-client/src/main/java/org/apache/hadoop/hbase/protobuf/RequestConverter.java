...
public final class RequestConverter {
  ...
  /**
   * Create a new protocol buffer GetRequest to get a row, all columns in a family. If there is no
   * such row, return the closest row before it.
   * @param regionName the name of the region to get
   * @param row the row to get
   * @param family the column family to get should return the immediate row before
   * @return a protocol buffer GetReuqest
   */
  public static GetRequest buildGetRowOrBeforeRequest(final byte[] regionName, final byte[] row,
      final byte[] family) {
  ...
  public static ClientProtos.MultiRequest buildMutateRequest(final byte[] regionName,
      ...
      final ByteArrayComparable comparator, final CompareType compareType,
    ...
    ClientProtos.MultiRequest request = ClientProtos.MultiRequest.newBuilder()
        .addRegionAction(builder.build()).setCondition(condition).build();
    return request;
  }
  ...
  /**
   * Create a protocol buffer MultiRequest for row mutations. Does not propagate Action absolute
   * position. Does not set atomic action on the created RegionAtomic. Caller should do that if
   * wanted.
   * @param regionName
   * @param rowMutations
  ...
  /**
   * Create a protocol buffer MultiRequest for row mutations that does not hold data. Data/Cells are
   * carried outside of protobuf. Return references to the Cells in <code>cells</code> param. Does
   * not propagate Action absolute position. Does not set atomic action on the created RegionAtomic.
   * Caller should do that if wanted.
   * @param regionName
   * @param rowMutations
  ...
  public static <R> RegionAction.Builder buildRegionAction(final byte[] regionName,
    ...
    for (Action<R> action : actions) {
      ...
      if (row instanceof Get) {
        ...
        regionActionBuilder.addAction(actionBuilder.setMutation(ProtobufUtil
            .toMutation(MutationType.APPEND, (Append) row, mutationBuilder, action.getNonce())));
        ...
        regionActionBuilder.addAction(actionBuilder.setServiceCall(
          ClientProtos.CoprocessorServiceCall.newBuilder().setRow(ByteStringer.wrap(exec.getRow()))
  ...
  public static <R> RegionAction.Builder buildNoDataRegionAction(final byte[] regionName,
    ...
    for (Action<R> action : actions) {
      ...
      if (row instanceof Get) {
        ...
        Append a = (Append) row;
        cells.add(a);
        builder.addAction(actionBuilder.setMutation(ProtobufUtil
            .toMutationNoData(MutationType.APPEND, a, mutationBuilder, action.getNonce())));
      } else if (row instanceof Increment) {
        Increment i = (Increment) row;
        cells.add(i);
        builder.addAction(actionBuilder.setMutation(ProtobufUtil
            .toMutationNoData(MutationType.INCREMENT, i, mutationBuilder, action.getNonce())));
      } else if (row instanceof RowMutations) {
  ...
  public static OpenRegionRequest buildOpenRegionRequest(ServerName server,
      final List<Triple<HRegionInfo, Integer, List<ServerName>>> regionOpenInfos,
  ...
  public static OpenRegionRequest buildOpenRegionRequest(ServerName server,
    ...
    OpenRegionRequest.Builder builder = OpenRegionRequest.newBuilder();
    builder.addOpenInfo(
      buildRegionOpenInfo(region, versionOfOfflineNode, favoredNodes, openForReplay));
    if (server != null) {
      builder.setServerStartCode(server.getStartcode());
    }
    return builder.build();
   ...
   */
  public static CloseRegionRequest buildCloseRegionRequest(ServerName server,
      final String encodedRegionName, final boolean transitionInZK) {
    CloseRegionRequest.Builder builder = CloseRegionRequest.newBuilder();
    RegionSpecifier region = buildRegionSpecifier(RegionSpecifierType.ENCODED_REGION_NAME,
   ...
   */
  public static MergeRegionsRequest buildMergeRegionsRequest(final byte[] regionA,
      final byte[] regionB, final boolean forcible) {
    MergeRegionsRequest.Builder builder = MergeRegionsRequest.newBuilder();
    RegionSpecifier regionASpecifier =
        buildRegionSpecifier(RegionSpecifierType.REGION_NAME, regionA);
    RegionSpecifier regionBSpecifier =
        buildRegionSpecifier(RegionSpecifierType.REGION_NAME, regionB);
  ...
  public static DispatchMergingRegionsRequest buildDispatchMergingRegionsRequest(
    ...
    builder.setRegionA(
      buildRegionSpecifier(RegionSpecifierType.ENCODED_REGION_NAME, encodedNameOfRegionA));
    builder.setRegionB(
      buildRegionSpecifier(RegionSpecifierType.ENCODED_REGION_NAME, encodedNameOfRegionB));
    builder.setForcible(forcible);
    return builder.build();
  }
   ...
   */
  public static CreateNamespaceRequest buildCreateNamespaceRequest(NamespaceDescriptor descriptor) {
   ...
   */
  public static ModifyNamespaceRequest buildModifyNamespaceRequest(NamespaceDescriptor descriptor) {
    return ModifyNamespaceRequest.newBuilder()
        .setNamespaceDescriptor(ProtobufUtil.toProtoNamespaceDescriptor(descriptor)).build();
  }
   ...
   */
  public static DeleteNamespaceRequest buildDeleteNamespaceRequest(String name) {
    return DeleteNamespaceRequest.newBuilder().setNamespaceName(name).build();
  }
   ...
   */
  public static GetNamespaceDescriptorRequest buildGetNamespaceDescriptorRequest(String name) {
    return GetNamespaceDescriptorRequest.newBuilder().setNamespaceName(name).build();
  }

  /**
   * Creates a protocol buffer ListTableDescriptorsByNamespaceRequest
   * @param name
   * @return a ListTableDescriptorsByNamespaceRequest
   */
  public static ListTableDescriptorsByNamespaceRequest buildListTableDescriptorsByNamespaceRequest(
      String name) {
    return ListTableDescriptorsByNamespaceRequest.newBuilder().setNamespaceName(name).build();
  }

  /**
   * Creates a protocol buffer ListTableNamesByNamespaceRequest
   * @param name
   * @return a ListTableNamesByNamespaceRequest
   */
  public static ListTableNamesByNamespaceRequest buildListTableNamesByNamespaceRequest(
      String name) {
    return ListTableNamesByNamespaceRequest.newBuilder().setNamespaceName(name).build();
  }
  ...
  public static GetLastFlushedSequenceIdRequest buildGetLastFlushedSequenceIdRequest(
      byte[] regionName) {
    return GetLastFlushedSequenceIdRequest.newBuilder().setRegionName(ByteStringer.wrap(regionName))
        .build();
  }
   ...
   */
  private static RegionOpenInfo buildRegionOpenInfo(final HRegionInfo region,
      final int versionOfOfflineNode, final List<ServerName> favoredNodes, Boolean openForReplay) {
  ...
  public static SwitchThrottleRequest buildSwitchThrottleRequest(ThrottleState state) {
    SwitchThrottleRequest.Builder request = SwitchThrottleRequest.newBuilder();
    request.setThrottleState(ProtobufUtil.toProtoThrottleState(state));
    return request.build();
  }

  public static IsSnapshotDoneRequest buildIsSnapshotDoneRequest(SnapshotDescription snapshot) {
    return IsSnapshotDoneRequest.newBuilder().setSnapshot(snapshot).build();
  }

  public static SnapshotRequest buildSnapshotRequest(SnapshotDescription snapshot) {
    return SnapshotRequest.newBuilder().setSnapshot(snapshot).build();
  }
  ...
  public static ProcedureDescription buildProcedureDescription(String signature, String instance,
    ...
    for (String key : props.keySet()) {
      builder.addConfiguration(NameStringPair.newBuilder().setName(key).setValue(props.get(key)));
    }
    return builder.build();
  }

  public static ExecProcedureRequest buildExecProcedureRequest(String signature, String instance,
      Map<String, String> props) {
    return ExecProcedureRequest.newBuilder()
        .setProcedure(buildProcedureDescription(signature, instance, props)).build();
  }

  public static IsProcedureDoneRequest buildIsProcedureDoneRequest(String signature,
      String instance, Map<String, String> props) {
    return IsProcedureDoneRequest.newBuilder()
        .setProcedure(buildProcedureDescription(signature, instance, props)).build();
  }

  public static IsRestoreSnapshotDoneRequest buildIsRestoreSnapshotDoneRequest(
