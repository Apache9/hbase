...
public class RpcClientImpl extends AbstractRpcClient {
  ...
  protected class Connection extends Thread {
    ...
    Connection(ConnectionId remoteId, final Codec codec, final CompressionCodec compressor)
      ...
      if (useSasl && securityInfo != null) {
        ...
        if (tokenKind != null) {
          TokenSelector<? extends TokenIdentifier> tokenSelector = TOKEN_HANDLERS.get(tokenKind);
      ...
      if (remoteId.rpcTimeout > 0) {
        ...
        pingInterval = RpcClientImpl.this.pingInterval;
      }
    ...
    /**
     * This class sends a ping to the remote side when timeout on reading. If no failure is
     * detected, it retries until at least a byte is read.
     */
     ...
     * retries, stop retrying and throw the exception; Otherwise backoff N seconds and try
     * connecting again. This Method is only called from inside setupIOstreams(), which is
    ...
    /**
     * If multiple clients with the same principal try to connect to the same server at the same
     * time, the server assumes a replay attack is in progress. This is a feature of kerberos. In
     * order to work around this, what is done is that the client backs off randomly and tries to
     * initiate the connection again. The other problem is to do with ticket expiry. To handle that,
     * a relogin is attempted.
     * <p>
     * The retry logic is governed by the {@link #shouldAuthenticateOverKrb} method. In case when
     * the user doesn't have valid credentials, we don't need to retry (from cache or ticket). In
     * such cases, it is prudent to throw a runtime exception when we receive a SaslException from
     * the underlying authentication implementation, so there is no retry from other high level (for
     * eg, HCM or HBaseAdmin).
     * </p>
     */
    private synchronized void handleSaslConnectionFailure(final int currRetries,
    ...
    protected synchronized void setupIOstreams() throws IOException, InterruptedException {
      ...
      if (failedServer != null) {
        if (LOG.isDebugEnabled()) {
          LOG.debug(
            "Not trying to connect to " + server + " this server is in the failed servers list");
        }
        IOException e = new FailedServerException("This server is in the failed servers list"
            + failedServer.formatFailedTimeStamp() + ": " + server, failedServer.cause);
        markClosed(e);
        close();
        throw e;
      }
    ...
    protected void readResponse() {
      ...
      try {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
            getName() + ": got response header " + TextFormat.shortDebugString(responseHeader)
  ...
  Pair<Message, CellScanner> call(MethodDescriptor md, Message param, CellScanner cells,
      ...
      throws InterruptedException, IOException {
    int callId;
    synchronized (RpcClientImpl.this) {
      callId = counter++;
    }
    Call call = new Call(callId, md, param, cells, returnType, rpcTimeout);
    Connection connection = getConnection(ticket, call, addr, rpcTimeout, this.codec,
      this.compressor);
  ...
  public void cancelConnections(ServerName sn, Throwable exc) {
    synchronized (connections) {
      for (Connection connection : connections.values()) {
        if (connection.isAlive() && connection.getRemoteAddress().getPort() == sn.getPort()
            && connection.getRemoteAddress().getHostName().equals(sn.getHostname())) {
          LOG.info("The server " + sn.getServerName() + " is dead - stopping the connection "
              + connection.remoteId);
   ...
   */
  protected Connection getConnection(User ticket, Call call, InetSocketAddress addr, int rpcTimeout,
