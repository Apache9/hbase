...
class RegionCoprocessorRpcChannelImpl implements RpcChannel {
  ...
  private CompletableFuture<Message> rpcCall(MethodDescriptor method, Message request,
    ...
    ClientProtos.CoprocessorServiceCall call = ClientProtos.CoprocessorServiceCall.newBuilder()
        .setRow(ByteStringer.wrap(row)).setServiceName(method.getService().getFullName())
        .setMethodName(method.getName()).setRequest(request.toByteString()).build();
    CoprocessorServiceRequest csr = CoprocessorServiceRequest.newBuilder().setCall(call)
        .setRegion(
          RequestConverter.buildRegionSpecifier(REGION_NAME, loc.getRegionInfo().getRegionName()))
        .build();
    stub.execService(controller, csr, new RpcCallback<CoprocessorServiceResponse>() {
      ...
      public void run(CoprocessorServiceResponse resp) {
        if (controller.failed()) {
          ...
          try {
            ...
            if (resp.getValue().hasValue()) {
              response = responsePrototype.newBuilderForType().mergeFrom(resp.getValue().getValue())
                  .build();
            } else {
