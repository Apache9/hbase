...
/**
 * For creating {@link BufferedMutatorBuilder}.
 */
@InterfaceAudience.Public
public interface BufferedMutatorBuilder {

  /**
   * Set timeout for the background flush operation.
   */
  BufferedMutatorBuilder setOperationTimeout(long timeout, TimeUnit unit);

  /**
   * Set timeout for each rpc request when doing background flush.
   */
  BufferedMutatorBuilder setRpcTimeout(long timeout, TimeUnit unit);

  /**
   * Set the base pause time for retrying. We use an exponential policy to generate sleep time when
   * retrying.
   */
  BufferedMutatorBuilder setRetryPause(long pause, TimeUnit unit);
   ...
   */
  default BufferedMutatorBuilder setMaxRetries(int maxRetries) {
   ...
   * @see #setOperationTimeout(long, TimeUnit)
   */
  BufferedMutatorBuilder setMaxAttempts(int maxAttempts);

  /**
   * Set the number of retries that are allowed before we start to log.
   */
  BufferedMutatorBuilder setStartLogErrorsCnt(int startLogErrorsCnt);
   ...
   */
  BufferedMutatorBuilder setWriteBufferSize(long writeBufferSize);

  /**
   * Create the {@link BufferedMutator} instance.
   */
  BufferedMutator build();
}
