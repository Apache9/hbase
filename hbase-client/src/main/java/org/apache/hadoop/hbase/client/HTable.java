...
/**
 * <p>
 * Used to communicate with a single HBase table. An implementation of {@link HTableInterface}.
 * Instances of this class can be constructed directly but it is encouraged that users get instances
 * via {@link HConnection} and {@link HConnectionManager}. See {@link HConnectionManager} class
 * comment for an example.
 * <p>
 * This class is not thread safe for reads nor write.
 * <p>
 * In case of writes (Put, Delete), the underlying write buffer can be corrupted if multiple threads
 * contend over a single HTable instance.
 * <p>
 * In case of reads, some fields used by a Scan are shared among all threads. The HTable
 * implementation can either not contract to be safe in case of a Get
 * <p>
 * Instances of HTable passed the same {@link Configuration} instance will share connections to
 * servers out on the cluster and to the zookeeper ensemble as well as caches of region locations.
 * This is usually a *good* thing and it is recommended to reuse the same configuration object for
 * all your tables. This happens because they will all share the same underlying {@link HConnection}
 * instance. See {@link HConnectionManager} for more on how this mechanism works.
 * <p>
 * {@link HConnection} will read most of the configuration it needs from the passed
 * {@link Configuration} on initial construction. Thereafter, for settings such as
 * <code>hbase.client.pause</code>, <code>hbase.client.retries.number</code>, and
 * <code>hbase.client.rpc.maxattempts</code> updating their values in the passed
 * {@link Configuration} subsequent to {@link HConnection} construction will go unnoticed. To run
 * with changed values, make a new {@link HTable} passing a new {@link Configuration} instance that
 * has the new configuration.
 * <p>
 * Note that this class implements the {@link Closeable} interface. When a HTable instance is no
 * longer required, it *should* be closed in order to ensure that the underlying resources are
...
public class HTable implements HTableInterface {
   ...
   * other HTable instances created with the same <code>conf</code> instance. Uses already-populated
   * region cache if one is available, populated by any other HTable instances sharing this
   * <code>conf</code> instance. Recommended.
   * @param conf Configuration object to use.
   * @param tableName Name of the table.
   * @throws IOException if a remote or network exception occurs It could be just the name of the
   *           table (the short form), the full uri name of the table with the hbase cluster
   *           information. For example: "table1": just the table name "hbase://lgprc-test/table1":
   *           "table1" on the hbase cluster "lgprc-test", which is located by the name service.
   *           "hbase://lgprc-test:9800/table1": the zookeeper cluster is running on port 9800
   *           instead of the default port (11000). When it's a full uri name, the cluster must be
   *           setup with kerberos authentication enabled. When it's a just a short form name, the
   *           configuration would be loaded from configuration files.
   */
  ...
  /**
   * The finishSetup should be false when running unit test, then it won't try to connect to the
   * hbase service.
   */
   ...
   * other HTable instances created with the same <code>conf</code> instance. Uses already-populated
   * region cache if one is available, populated by any other HTable instances sharing this
   * <code>conf</code> instance. Use this constructor when the ExecutorService is externally
   * managed.
   * @param conf Configuration object to use.
   * @param tableName Name of the table.
  ...
  public HTable(TableName tableName, final HConnection connection, final ExecutorService pool)
      throws IOException {
    this(tableName, connection, null, pool);
  }
  ...
  public HTable(TableName tableName, final HConnection connection,
      final TableConfiguration tableConfig, final ExecutorService pool) throws IOException {
  ...
  public NavigableMap<HRegionInfo, ServerName> getRegionLocations() throws IOException {
    // TODO: Odd that this returns a Map of HRI to SN whereas getRegionLocation, singular, returns
    // an HRegionLocation.
  ...
  public void delete(final Delete delete) throws IOException {
    connection.getRpcRetryingCallerFactory().<Boolean> newCaller()
        .callWithRetries(new RegionServerCallable<Boolean>(connection, tableName, delete.getRow()) {
  ...
  public void mutateRow(final RowMutations rm) throws IOException {
    connection.getRpcRetryingCallerFactory().<Void> newCaller()
        .callWithRetries(new RegionServerCallable<Void>(connection, getName(), rm.getRow()) {

          @Override
          public Void rpcCall() throws IOException {
            try {
              if (Trace.isTracing()) {
                Trace.addTimelineAnnotation("MutateRow to " + location);
              }
              RegionAction.Builder regionMutationBuilder = RequestConverter
                  .buildRegionAction(getLocation().getRegionInfo().getRegionName(), rm);
              regionMutationBuilder.setAtomic(true);
              MultiRequest request =
                  MultiRequest.newBuilder().addRegionAction(regionMutationBuilder.build()).build();
              ClientProtos.MultiResponse response = getStub().multi(getController(), request);
  ...
  public Result append(final Append append) throws IOException {
    ...
    return connection.getRpcRetryingCallerFactory().<Result> newCaller().callWithRetries(
      new RegionServerCallable<Result>(this.connection, getName(), append.getRow()) {
        ...
        @Override
        protected Result rpcCall() throws ServiceException, IOException {
          if (Trace.isTracing()) {
          ...
          return ProtobufUtil.toResult(response.getResult(), getController().cellScanner());
        }
      }, this.operationTimeout);
  }
  ...
  public Result increment(final Increment increment) throws IOException {
    ...
    final long nonceGroup = ng.getNonceGroup(), nonce = ng.newNonce();
    return connection.getRpcRetryingCallerFactory().<Result> newCaller().callWithRetries(
      new RegionServerCallable<Result>(this.connection, getName(), increment.getRow()) {
  ...
  public long incrementColumnValue(final byte[] row, final byte[] family, final byte[] qualifier,
    ...
    NonceGenerator ng = this.connection.getNonceGenerator();
    final long nonceGroup = ng.getNonceGroup(), nonce = ng.newNonce();
    return connection.getRpcRetryingCallerFactory().<Long> newCaller()
        .callWithRetries(new RegionServerCallable<Long>(connection, getName(), row) {

          @Override
          protected Long rpcCall() throws ServiceException, IOException {
            if (Trace.isTracing()) {
            ...
            MutateRequest request = RequestConverter.buildIncrementRequest(
              ...
              durability, nonceGroup, nonce);
            MutateResponse response = getStub().mutate(getController(), request);
            Result result =
                ProtobufUtil.toResult(response.getResult(), getController().cellScanner());
  ...
  public boolean checkAndPut(final byte[] row, final byte[] family, final byte[] qualifier,
    ...
    return connection.getRpcRetryingCallerFactory().<Boolean> newCaller()
        .callWithRetries(new RegionServerCallable<Boolean>(connection, getName(), row) {
          ...
          protected Boolean rpcCall() throws ServiceException, IOException {
            ...
            MutateRequest request =
                RequestConverter.buildMutateRequest(getLocation().getRegionInfo().getRegionName(),
                  row, family, qualifier, new BinaryComparator(value), CompareType.EQUAL, put);
            MutateResponse response = getStub().mutate(getController(), request);
            return Boolean.valueOf(response.getProcessed());
          }
        }, this.operationTimeout);
  ...
  public boolean checkAndDelete(final byte[] row, final byte[] family, final byte[] qualifier,
    ...
    return connection.getRpcRetryingCallerFactory().<Boolean> newCaller()
        .callWithRetries(new RegionServerCallable<Boolean>(connection, getName(), row) {
          ...
          @Override
          protected Boolean rpcCall() throws IOException {
            try {
              if (Trace.isTracing()) {
              ...
              MutateRequest request =
                  RequestConverter.buildMutateRequest(getLocation().getRegionInfo().getRegionName(),
                    row, family, qualifier, new BinaryComparator(value), CompareType.EQUAL, delete);
              MutateResponse response = getStub().mutate(getController(), request);
              return Boolean.valueOf(response.getProcessed());
            } catch (ServiceException se) {
              throw ProtobufUtil.getRemoteException(se);
    ...
    return connection.getRpcRetryingCallerFactory().<Boolean> newCaller()
        .callWithRetries(new RegionServerCallable<Boolean>(connection, getName(), row) {
          ...
          protected Boolean rpcCall() throws ServiceException, IOException {
            ...
            MultiRequest request =
                RequestConverter.buildMutateRequest(getLocation().getRegionInfo().getRegionName(),
                  row, family, qualifier, new BinaryComparator(value), compareType, rm);
            ClientProtos.MultiResponse response = getStub().multi(getController(), request);
            ClientProtos.RegionActionResult res = response.getRegionActionResultList().get(0);
            if (res.hasException()) {
              Throwable ex = ProtobufUtil.toException(res.getException());
            ...
            new AsyncProcess.AsyncProcessCallback<ClientProtos.CoprocessorServiceResult>() {
              ...
              public void success(int originalIndex, byte[] region, Row row,
                ...
                if (LOG.isTraceEnabled()) {
                  LOG.trace("Received result for endpoint " + methodDescriptor.getFullName()
                      ...
                      + ", row=" + Bytes.toStringBinary(row.getRow()) + ", value="
                      + serviceResult.getValue().getValue());
                }
                try {
                  callback.update(region, row.getRow(), (R) responsePrototype.newBuilderForType()
                      .mergeFrom(serviceResult.getValue().getValue()).build());
                } catch (InvalidProtocolBufferException e) {
                  LOG.error(
                    "Unexpected response type from endpoint " + methodDescriptor.getFullName(), e);
              ...
              public boolean failure(int originalIndex, byte[] region, Row row, Throwable t) {
                ...
                LOG.error("Failed calling endpoint " + methodDescriptor.getFullName() + ": region="
                    + Bytes.toStringBinary(exec.getRegion()),
                  t);
                return true;
              }
