...
class AsyncScanSingleRegionRpcRetryingCaller {
  ...
  private void closeScanner() {
    ...
    ScanRequest req = RequestConverter.buildScanRequest(this.scannerId, 0, true);
  ...
  private void renewLease() {
    ...
    nextCallSeq++;
    resetController(controller, rpcTimeoutNs);
    ScanRequest req = RequestConverter.buildScanRequest(scannerId, 0, false, nextCallSeq, true, -1);
    stub.scan(controller, req, resp -> {
    });
  }
