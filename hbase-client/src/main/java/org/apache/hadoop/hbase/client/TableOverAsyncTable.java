...
class TableOverAsyncTable implements Table {
  ...
  private final RawAsyncTableImpl table;

  private final ExecutorService pool;

  TableOverAsyncTable(AsyncConnectionImpl conn, RawAsyncTableImpl table, ExecutorService pool) {
    this.conn = conn;
    this.table = table;
    this.pool = pool;
  }
  ...
  public TableDescriptor getDescriptor() throws IOException {
    return get(conn.getAdmin().getDescriptor(getName()));
  }

  @Override
  public boolean exists(Get get) throws IOException {
    return get(table.exists(get));
  }

  @Override
  public boolean[] exists(List<Get> gets) throws IOException {
    return Booleans.toArray(get(table.existsAll(gets)));
  }

  @Override
  public void batch(List<? extends Row> actions, Object[] results)
      throws IOException, InterruptedException {
    List<Object> list = get(table.batchAll(actions));
  ...
  public Result get(Get get) throws IOException {
    return get(table.get(get));
  }

  @Override
  public Result[] get(List<Get> gets) throws IOException {
    return get(table.getAll(gets)).toArray(new Result[0]);
  }
  ...
  public void put(List<Put> puts) throws IOException {
    get(table.putAll(puts));
  }

  @Override
  public void delete(Delete delete) throws IOException {
    get(table.delete(delete));
  }

  @Override
  public void delete(List<Delete> deletes) throws IOException {
    get(table.deleteAll(deletes));
  }
  ...
  private static final class CheckAndMutateBuilderImpl implements CheckAndMutateBuilder {
    ...
    public boolean thenPut(Put put) throws IOException {
      return get(builder.thenPut(put));
    }

    @Override
    public boolean thenDelete(Delete delete) throws IOException {
      return get(builder.thenDelete(delete));
    }

    @Override
    public boolean thenMutate(RowMutations mutation) throws IOException {
      return get(builder.thenMutate(mutation));
    }
  }
  ...
  public void mutateRow(RowMutations rm) throws IOException {
    get(table.mutateRow(rm));
  }

  @Override
  public Result append(Append append) throws IOException {
    return get(table.append(append));
  }

  @Override
  public Result increment(Increment increment) throws IOException {
    return get(table.increment(increment));
  }

  @Override
  public long incrementColumnValue(byte[] row, byte[] family, byte[] qualifier, long amount)
      throws IOException {
    return get(table.incrementColumnValue(row, family, qualifier, amount));
  }

  @Override
  public long incrementColumnValue(byte[] row, byte[] family, byte[] qualifier, long amount,
      Durability durability) throws IOException {
    return get(table.incrementColumnValue(row, family, qualifier, amount, durability));
  }
  ...
  private static final class RegionCoprocessorRpcChannel extends RegionCoprocessorRpcChannelImpl
    ...
    public Message callBlockingMethod(MethodDescriptor method, RpcController controller,
      ...
      if (c.failed()) {
        controller.setFailed(c.errorText());
  ...
  private Pair<List<byte[]>, List<HRegionLocation>> getKeysAndRegionsInRange(final byte[] startKey,
    ...
    do {
      HRegionLocation regionLocation =
        get(conn.getRegionLocator(getName()).getRegionLocation(currentKey, reload));
