...
module Shell
  ...
  Notice:
  1. The replicate_all flag need to be true when you want set exclude table-cfs.
  2. Can't set a exclude table if it's namespace was already in exclude namespaces config
     of this peer.
  ...
  EOF
      ...
      def command(id, exclude_table_cfs = nil)
        format_simple_command do
          replication_admin.set_peer_exclude_tableCFs(id, exclude_table_cfs)
        end
      end
    end
  end
