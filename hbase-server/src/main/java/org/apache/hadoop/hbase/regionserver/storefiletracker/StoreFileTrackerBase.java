...
abstract class StoreFileTrackerBase implements StoreFileTracker {
  ...
  public final StoreFileWriter createWriter(CreateStoreFileWriterParams params)
    ...
    long totalCompactedFilesSize = params.getTotalCompactedFilesSize();
    ...
    HFileContext hFileContext = createFileContext(params.getCompression(),
      params.isIncludeMVCCReadpoint(), params.isIncludesTag(), encryptionContext);
    Path outputDir;
    ...
    StoreFileWriter.Builder builder =
      new StoreFileWriter.Builder(conf, writerCacheConf, ctx.getRegionFileSystem().getFileSystem())
        ...
        .withMaxKeyCount(params.getMaxKeyCount()).withFavoredNodes(ctx.getFavoredNodes())
        .withFileContext(hFileContext).withShouldDropCacheBehind(params.isShouldDropBehind())
        .withCompactedFilesSupplier(ctx.getCompactedFilesSupplier())
        .withFileStoragePolicy(params.getFileStoragePolicy());
