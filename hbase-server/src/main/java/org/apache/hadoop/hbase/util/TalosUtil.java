...
public class TalosUtil {
  ...
  public static List<SimpleMutation> convertMessageToMutation(Message message) throws IOException {
    ...
    try {
      ...
      for (ByteString kv : kvs) {
        Cell cell = ProtobufUtil.toCell(builder, CellProtos.Cell.parseFrom(kv));
        if (CellUtil.matchingFamily(cell, METAFAMILY)) {
          continue;
        }
        if (isNewRowOrType(prevCell, cell)) {
          m = new SimpleMutation(CellUtil.cloneRow(cell), cell.getType().getCode());
          mutations.add(m);
        }
  ...
  public static List<Result> convertMessageToResult(Message message,
    ...
    try {
      ...
      for (ByteString kv : kvs) {
        ...
        if (prevCell == null) {
          prevCell = cell;
        } else if (CellComparator.getInstance().compareRows(prevCell, cell) != 0) {
  ...
  public static List<Message> constructMessages(WAL.Entry entry) throws IOException {
  ...
  public static byte[] getMessageByteArray(WAL.Entry entry) throws IOException {
