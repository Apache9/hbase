...
public class FSHLog extends AbstractFSHLog<Writer> {
  ...
  protected long doReplaceWriter(Path oldPath, Path newPath, Writer nextWriter) throws IOException {
    ...
    SafePointZigZagLatch zigzagLatch =
        (this.ringBufferEventHandler == null) ? null : this.ringBufferEventHandler
            .attainSafePoint();
  ...
  class RingBufferEventHandler implements EventHandler<RingBufferTruck>, LifecycleAware {
    ...
    void append(final FSWALEntry entry) throws Exception {
      try {
        ...
        String msg =
            "Append sequenceId=" + entry.getKey().getLogSeqNum() + ", requesting roll of WAL";
        LOG.warn(msg, e);
        requestLogRoll();
