...
public class StripeCompactionScanQueryMatcher extends DropDeletesCompactionScanQueryMatcher {
  ...
  public StripeCompactionScanQueryMatcher(ScanInfo scanInfo, DeleteTracker deletes, ColumnTracker columns,
  ...
  @Override
  public MatchCode match(KeyValue cell) throws IOException {
  ...
  private boolean entered() {
    return dropDeletesFromRow.length == 0
        || rowComparator.compareRows(currentRow.getRowArray(), currentRow.getRowOffset(),
          currentRow.getRowLength(), dropDeletesFromRow, 0, dropDeletesFromRow.length) >= 0;
  }

  private boolean left() {
    return dropDeletesToRow.length > 0
        && rowComparator.compareRows(currentRow.getRowArray(), currentRow.getRowOffset(),
          currentRow.getRowLength(), dropDeletesToRow, 0, dropDeletesToRow.length) >= 0;
  }
