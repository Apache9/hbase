...
public class TransitPeerSyncReplicationStateProcedure
  ...
  private void reopenRegions(MasterProcedureEnv env) {
    addChildProcedure(env.getReplicationPeerManager().getPeerConfig(peerId).get().getTableCFsMap()
      .keySet().stream().map(tn -> new ReopenTableRegionsProcedure(tn))
      .toArray(ReopenTableRegionsProcedure[]::new));
  }
