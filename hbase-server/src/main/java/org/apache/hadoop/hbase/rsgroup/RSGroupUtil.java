...
public final class RSGroupUtil {
  ...
  public static boolean checkRSGroupNotEmpty(RSGroupInfo rsGroupInfo) {
    if (!rsGroupInfo.getServers().isEmpty()) {
      return true;
    }
    Integer requiredServerNum = RSGroupNormalizerChore.getRequiredServerNum(rsGroupInfo);
    if (requiredServerNum != null && requiredServerNum > 0) {
      return true;
    }
    return false;
  }
