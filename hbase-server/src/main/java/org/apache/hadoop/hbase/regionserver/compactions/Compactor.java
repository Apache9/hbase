...
public abstract class Compactor<T extends CellSink> {
  ...
  protected final CreateStoreFileWriterParams createParams(FileDetails fd, boolean shouldDropBehind,
    boolean major) {
    return CreateStoreFileWriterParams.create().setMaxKeyCount(fd.maxKeyCount)
      .setCompression(major ? majorCompactionCompression : minorCompactionCompression)
      .setCompaction(true).setIncludeMVCCReadpoint(fd.maxMVCCReadpoint > 0)
      .setIncludesTag(fd.maxTagsLength > 0).setShouldDropBehind(shouldDropBehind)
      .setTotalCompactedFilesSize(fd.totalCompactedFilesSize);
  }
  ...
  protected final StoreFileWriter createWriter(FileDetails fd, boolean shouldDropBehind,
    ...
    return store.getStoreEngine().getStoreFileTracker().createWriter(
      createParams(fd, shouldDropBehind, major).setFileStoragePolicy(fileStoragePolicy));
  }
