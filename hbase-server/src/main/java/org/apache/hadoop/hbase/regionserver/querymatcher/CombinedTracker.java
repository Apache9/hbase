...
public class CombinedTracker implements ColumnTracker, DeleteTracker {
  ...
  protected class DeleteVersionsNode {
    ...
    public void addVersionDelete(Cell cell) {
      ...
      // masked.
      SortedSet<Long> nextValue = mvccCountingMap.ceilingEntry(cell.getMvccVersion()).getValue();
      SortedSet<Long> thisValue = new TreeSet<>(nextValue.headSet(cell.getMvccVersion()));
      mvccCountingMap.put(cell.getMvccVersion(), thisValue);
    }
  ...
  protected long prepare(Cell cell) {
    ...
    if (matchCq && !KeyValue.isDelete(lastCqType) && lastCqType == cell.getTypeByte()
    ...
    lastCqTs = cell.getTimestamp();
    lastCqMvcc = cell.getMvccVersion();
  ...
  public void add(Cell cell) {
    ...
    case DeleteFamily: // Delete all versions of all columns of the specified family
      delFamMap.put(cell.getMvccVersion(),
          new DeleteVersionsNode(cell.getTimestamp(), cell.getMvccVersion()));
      break;
    case DeleteFamilyVersion: // Delete all columns of the specified family and specified version
      delFamMap.ceilingEntry(cell.getMvccVersion()).getValue().addVersionDelete(cell);
      break;

    // These two kinds of markers are mix with Puts.
    case DeleteColumn: // Delete all versions of the specified column
      delColMap.put(cell.getMvccVersion(),
          new DeleteVersionsNode(cell.getTimestamp(), cell.getMvccVersion()));
      break;
    case Delete: // Delete the specified version of the specified column.
      delColMap.ceilingEntry(cell.getMvccVersion()).getValue().addVersionDelete(cell);
      break;
    default:
  ...
  public DeleteResult isDeleted(Cell cell) {
    ...
    for (Map.Entry<Long, DeleteVersionsNode> e : delColMap.tailMap(cell.getMvccVersion())
        .entrySet()) {
      ...
      if (deleteVersionMvccs != null) {
        SortedSet<Long> tail = deleteVersionMvccs.tailSet(cell.getMvccVersion());
        if (!tail.isEmpty()) {
