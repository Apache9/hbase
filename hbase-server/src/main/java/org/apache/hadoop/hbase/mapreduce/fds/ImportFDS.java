...
public class ImportFDS {
  private static final Log LOG = LogFactory.getLog(ImportFDS.class);
  ...
  public static Job createSubmittable(Configuration conf, String[] args) throws IOException {
    ...
    HTable table = new HTable(conf, conf.get(FDS_OUTPUT_TABLE_NAME));
    ...
    // since 2.6 yarn cluster is not compatible with 2.4 mapreduce client, remove related jars...
    removeMapreduceJar(job.getConfiguration());
    return job;
  }
