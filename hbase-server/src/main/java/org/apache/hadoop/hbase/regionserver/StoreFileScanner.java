...
/**
 * KeyValueScanner adaptor over the Reader. It also provides hooks into bloom
 * filter things.
 */
...
public class StoreFileScanner implements KeyValueScanner {
  ...
  public void setCompaction(boolean isCompaction) {
    ...
    public static List<StoreFileScanner> getScannersForStoreFiles(
        ...
        return getScannersForStoreFiles(files, cacheBlocks, usePread,
                isCompaction, null, readPt);
    }

    /**
     * Return an array of scanners corresponding to the given set of store
     * files, And set the ScanQueryMatcher for each store file scanner for
     * further optimization
     */
    public static List<StoreFileScanner> getScannersForStoreFiles(
            Collection<StoreFile> files, boolean cacheBlocks, boolean usePread,
            boolean isCompaction, ScanQueryMatcher matcher, long readPt)
            throws IOException {
        List<StoreFileScanner> scanners = new ArrayList<StoreFileScanner>(
                files.size());
        for (StoreFile file: files) {
            StoreFile.Reader r = file.createReader();
            StoreFileScanner scanner = r.getStoreFileScanner(cacheBlocks,
                    usePread, isCompaction, readPt);
  ...
  public KeyValue next() throws IOException {
    ...
    public boolean reseek(KeyValue key) throws IOException {
        ...
        try {
            ...
            throw new IOException(
                    "Could not reseek " + this + " to key " + key, ioe);
        }
    }
        ...
        while (enforceMVCC && cur != null && (cur.getMvccVersion() > readPt)) {
            ...
            if (this.stopSkippingKVsIfNextRow
                    && getComparator().compareRows(cur.getBuffer(),
                            ...
                            startKV.getBuffer(), startKV.getRowOffset(),
    ...
    public static boolean seekAtOrAfter(HFileScanner s, KeyValue k)
            throws IOException {
        int result = s
                .seekTo(k.getBuffer(), k.getKeyOffset(), k.getKeyLength());
        if (result < 0) {
            if (result == HConstants.INDEX_KEY_MAGIC) {
                // using faked key
                return true;
            }
            // Passed KV is smaller than first KV in file, work from start of
            // file
            return s.seekTo();
        } else if (result > 0) {
    ...
    static boolean reseekAtOrAfter(HFileScanner s, KeyValue k)
        ...
        int result = s.reseekTo(k.getBuffer(), k.getKeyOffset(),
                k.getKeyLength());
        if (result <= 0) {
            if (result == HConstants.INDEX_KEY_MAGIC) {
                // using faked key
            ...
            // If up to now scanner is not seeked yet, this means passed KV is
            // smaller
            // than first KV in file, and it is the first time we seek on this
            // file.
     ...
     * that we find requested columns in more recent files and won't have to
     * seek in older files. Creates a fake key/value with the given row/column
     * and the highest (most recent) possible timestamp we might get from this
     * file. When users of such "lazy scanner" need to know the next KV
     * precisely (e.g. when this scanner is at the top of the heap), they run
     * {@link #enforceSeek()}.
     * <p>
     * Note that this function does guarantee that the current KV of this
     * scanner will be advanced to at least the given KV. Because of this, it
     * does have to do a real seek in cases when the seek timestamp is older
     * than the highest timestamp of the file, e.g. when we are trying to seek
    ...
    public boolean requestSeek(KeyValue kv, boolean forward, boolean useBloom)
        ...
        if (haveToSeek) {
            // This row/column might be in this store file (or we did not use
            // the
            ...
            if (seekTimestamp > maxTimestampInFile) {
                ...
                // row/column, and we don't know exactly what they are, so we
                // set the
                // seek key's timestamp to OLDEST_TIMESTAMP to skip the rest of
                // this
                // row/column.
                enforceSeek();
            }
        ...
        // top
        // of the KeyValueHeap in StoreScanner after we scanned this row/column
        // in
        // all other store files. The query matcher will then just skip this
        // fake
        // key/value and the store scanner will progress to the next column.
        // This
        // is obviously not a "real real" seek, but unlike the fake KV earlier
        // in
    ...
    public boolean seekToPreviousRow(KeyValue key) throws IOException {
        try {
            try {
                ...
                if (seekCount != null)
                    seekCount.incrementAndGet();
                if (!hfs.seekBefore(seekKey.getBuffer(),
                        seekKey.getKeyOffset(), seekKey.getKeyLength())) {
                    close();
                    return false;
                }
                ...
                if (!resultOfSkipKVs
                        || getComparator().compareRows(cur.getBuffer(),
                                cur.getRowOffset(), cur.getRowLength(),
                                firstKeyOfPreviousRow.getBuffer(),
            ...
            throw new IOException("Could not seekToPreviousRow " + this
                    + " to key " + key, ioe);
        }
    }
    ...
    public boolean backwardSeek(KeyValue key) throws IOException {
        ...
        if (cur == null
                || getComparator().compareRows(cur.getRowArray(),
                        cur.getRowOffset(), cur.getRowLength(),
                        key.getRowArray(), key.getRowOffset(),
                        key.getRowLength()) > 0) {
