...
public class MultiTableSnapshotInputFormatImpl {
   ...
   * restoreDir. Sets: {@link #RESTORE_DIRS_KEY}, {@link #SNAPSHOT_TO_SCANS_KEY}
   * @param conf
   * @param snapshotScans
   * @param restoreDir
   * @throws IOException
  ...
  public Map<String, Collection<Scan>> getSnapshotsToScans(Configuration conf) throws IOException {
    ...
    for (Map.Entry<String, String> entry : ConfigUtil.getKeyValues(conf, SNAPSHOT_TO_SCANS_KEY)) {
      String snapshotName = entry.getKey();
  ...
  public void setSnapshotToScans(Configuration conf, Map<String, Collection<Scan>> snapshotScans)
    ...
    ConfigUtil.setKeyValues(conf, SNAPSHOT_TO_SCANS_KEY, snapshotToSerializedScans);
  }
  ...
  public Map<String, Path> getSnapshotDirs(Configuration conf) throws IOException {
    List<Map.Entry<String, String>> kvps = ConfigUtil.getKeyValues(conf, RESTORE_DIRS_KEY);
  ...
  public void setSnapshotDirs(Configuration conf, Map<String, Path> snapshotDirs) {
    ...
    ConfigUtil.setKeyValues(conf, RESTORE_DIRS_KEY, toSet.entrySet());
  }
