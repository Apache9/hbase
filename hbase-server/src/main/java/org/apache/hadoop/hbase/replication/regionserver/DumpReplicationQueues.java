...
public class DumpReplicationQueues extends Configured implements Tool {
  ...
  public String dumpReplicationSummary() {
    ...
    sb.append("    Total size of WALs on HDFS: " + humanSize(totalSizeOfWALs) + "\n");
  ...
  public String dumpQueues(HConnection connection, ZooKeeperWatcher zkw, Set<String> peerIds,
    ...
    queuesClient = ReplicationFactory.getReplicationQueuesClient(zkw, getConf(),
      new WarnOnlyAbortable());
    queuesClient.init();
    replicationQueues = ReplicationFactory.getReplicationQueues(zkw, getConf(),
      new WarnOnlyAbortable());
    ...
    try {
      ...
      for (String regionserver : regionservers) {
        ...
        boolean isDead = !liveRegionServers.contains(regionserver);
        if (isDead) {
  ...
  private String formatQueue(String regionserver, ReplicationQueues replicationQueues, ReplicationQueueInfo queueInfo,
    ...
    if (hdfs) {
      ...
      sb.append("    Total size of WALs on HDFS for this queue: "
          + humanSize(getTotalWALSize(fs, wals, regionserver)) + "\n");
    }
    return sb.toString();
  }
  ...
  private long getTotalWALSize(FileSystem fs, List<String> wals, String server) throws IOException {
    ...
    for (String wal : wals) {
      try {
        Path oldLogPath = new Path(new Path(rootDir, HConstants.HREGION_OLDLOGDIR_NAME), wal);
        Path logPath = new Path(new Path(new Path(rootDir, HConstants.HREGION_LOGDIR_NAME), server), wal);
        fileStatus = (new FileLink(oldLogPath, logPath)).getFileStatus(fs);
      } catch (IOException e) {
  ...
  private String humanSize(double size) {
