...
public class RSRpcServices extends HBaseRpcServicesBase<HRegionServer>
  implements AdminService.BlockingInterface, ClientService.BlockingInterface {
  ...
  void start(ZKWatcher zkWatcher) {
    this.scannerIdGenerator = new ScannerIdGenerator(this.server.serverName);
  ...
  protected void checkOpen() throws IOException {
    if (server.isAborted()) {
      throw new RegionServerAbortedException("Server " + server.serverName + " aborting");
    }
    if (server.isStopped()) {
      throw new RegionServerStoppedException("Server " + server.serverName + " stopping");
    }
    if (!server.isDataFileSystemOk()) {
      throw new RegionServerStoppedException("File system not available");
    }
    if (!server.isOnline()) {
      throw new ServerNotRunningYetException("Server " + server.serverName
          + " is not running yet");
    }
  }
  ...
  public CompactionSwitchResponse compactionSwitch(RpcController controller,
    ...
    try {
      checkOpen();
      requestCount.increment();
      boolean prevState = server.compactSplitThread.isCompactionsEnabled();
      ...
      server.compactSplitThread.switchCompaction(request.getEnabled());
      return response;
    } catch (IOException ie) {
      throw new ServiceException(ie);
    }
    ...
    if (clearCompactionQueues.compareAndSet(false,true)) {
      try {
        ...
        for (String queueName : request.getQueueNameList()) {
          ...
          switch (queueName) {
            case "long":
              server.compactSplitThread.clearLongCompactionsQueue();
              break;
            case "short":
              server.compactSplitThread.clearShortCompactionsQueue();
              break;
            default:
  ...
  public GetServerInfoResponse getServerInfo(final RpcController controller,
    ...
    requestCount.increment();
    int infoPort = server.infoServer != null ? server.infoServer.getPort() : -1;
    return ResponseConverter.buildGetServerInfoResponse(server.serverName, infoPort);
  }
  ...
  private void throwOnWrongStartCode(long serverStartCode) throws ServiceException {
    ...
    if (server.serverName.getStartcode() != serverStartCode) {
      throw new ServiceException(new DoNotRetryIOException(
        "This RPC was intended for a " + "different server with startCode: " + serverStartCode +
          ", this server is: " + server.serverName));
    }
  }
      ...
      synchronized (server.online) {
        try {
          while (System.currentTimeMillis() <= endTime
            ...
            server.online.wait(server.msgInterval);
          }
          checkOpen();
    ...
    for (RegionOpenInfo regionOpenInfo : request.getOpenInfoList()) {
      ...
      try {
        ...
        if (previous == null || !previous.booleanValue()) {
          ...
          if (htd == null) {
            htd = server.tableDescriptors.get(region.getTable());
          ...
          if (server.executorService == null) {
            ...
            if (region.isMetaRegion()) {
              server.executorService.submit(new OpenMetaHandler(
              server, server, region, htd, masterSystemTime));
            } else {
              ...
              if (htd.getPriority() >= HConstants.ADMIN_QOS || region.getTable().isSystemTable()) {
                server.executorService.submit(new OpenPriorityRegionHandler(
                server, server, region, htd, masterSystemTime));
              } else {
                server.executorService.submit(new OpenRegionHandler(
                server, server, region, htd, masterSystemTime));
              }
            }
          }
  ...
  public BulkLoadHFileResponse bulkLoadHFile(final RpcController controller,
    ...
    if(clusterIds.contains(this.server.clusterId)){
      return BulkLoadHFileResponse.newBuilder().setLoaded(true).build();
    } else {
      clusterIds.add(this.server.clusterId);
    }
    try {
      checkOpen();
      requestCount.increment();
  ...
  private void executeOpenRegionProcedures(OpenRegionRequest request,
    ...
    for (RegionOpenInfo regionOpenInfo : request.getOpenInfoList()) {
      ...
      if (server.submitRegionProcedure(procId)) {
        server.executorService.submit(AssignRegionHandler
  ...
  private void executeCloseRegionProcedures(CloseRegionRequest request) {
    ...
    if (server.submitRegionProcedure(procId)) {
      server.executorService.submit(UnassignRegionHandler
