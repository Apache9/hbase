...
import org.apache.hadoop.hbase.coprocessor.SpecialRegionObserverForTraceCluster;
...
public class TestOpenTSDBCompactRegionObserver {
  ...
  public static void setUpBeforeClass() throws Exception {
    ...
    TEST_UTIL.waitTableAvailable(TSDB.getName());
  }
  ...
  public void test() throws Exception {
    loadData(TSDB);
    try (HTable table = new HTable(TEST_UTIL.getConfiguration(), TSDB)) {
    ...
    TEST_UTIL.getHBaseAdmin().flush(TSDB.getNameAsString());
    Thread.sleep(10000); // Sleep 1secs wait for flush
    TEST_UTIL.getHBaseAdmin().compact(TSDB.getNameAsString());
  ...
  private void loadData(TableName tableName) throws Exception {
    try (HTable table = new HTable(TEST_UTIL.getConfiguration(), tableName)) {
      table.put(new Put(ROWKEY_1).add(CF, CQ_1, now, VALUE_1));
      TEST_UTIL.getHBaseAdmin().flush(tableName.getNameAsString());
      Thread.sleep(10000); // Sleep 1secs wait for flush
      table.put(new Put(ROWKEY_1).add(CF, CQ_2, now, VALUE_2));
      // one datapoint
      table.put(new Put(ROWKEY_2).add(CF, CQ_3, now, VALUE_3));
    }
  }
}
