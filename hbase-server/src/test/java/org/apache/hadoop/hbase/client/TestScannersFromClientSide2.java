...
public class TestScannersFromClientSide2 {
  ...
  public static void setUp() throws Exception {
    ...
    try (HTable table = TEST_UTIL.createTable(TABLE_NAME, FAMILY, splitKeys)) {
      List<Put> puts = new ArrayList<>();
      for (int i = 0; i < 1000; i++) {
        puts.add(new Put(Bytes.toBytes(String.format("%03d", i))).add(FAMILY, CQ1, Bytes.toBytes(i))
            .add(FAMILY, CQ2, Bytes.toBytes(i * i)));
      }
  ...
  private List<Result> doScan(Scan scan) throws IOException {
    ...
    try (HTableInterface table = CONN.getTable(TABLE_NAME);
