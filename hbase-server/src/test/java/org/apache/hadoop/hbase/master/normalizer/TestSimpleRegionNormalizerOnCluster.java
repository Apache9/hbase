...
public class TestSimpleRegionNormalizerOnCluster {
  ...
  public void testRegionNormalizationSplitOnCluster() throws Exception {
    ...
    try (HTable ht = TEST_UTIL.createMultiRegionTable(TABLENAME, FAMILYNAME, 5)) {
      ...
      for(HRegion region: generatedRegions){
        LOG.info("region name: " + region.getRegionNameAsString() + Bytes.toStringBinary(region.getStartKey()));
      }
    ...
    assertEquals(5, MetaReader.getRegionCount(TEST_UTIL.getConfiguration(),TABLENAME.getNameAsString()));

    // Now trigger a split and stop when the split is in progress
    Thread.sleep(5000); // to let region load to update
    m.normalizeRegions();

    while (MetaReader.getRegionCount(TEST_UTIL.getConfiguration(),TABLENAME.getNameAsString()) < 6) {
      LOG.info("Waiting for normalization split to complete");
      Thread.sleep(100);
    }

    assertEquals(6, MetaReader.getRegionCount(TEST_UTIL.getConfiguration(),TABLENAME.getNameAsString()));
  ...
  public void testRegionNormalizationMergeOnCluster() throws Exception {
    ...
    while (MetaReader.getRegionCount(TEST_UTIL.getConfiguration(),TABLENAME.getNameAsString()) > 4) {
      LOG.info("Waiting for normalization merge to complete");
      Thread.sleep(100);
    }

    assertEquals(4, MetaReader.getRegionCount(TEST_UTIL.getConfiguration(),TABLENAME.getNameAsString()));

    admin.disableTable(TABLENAME);
    admin.deleteTable(TABLENAME);
  }

  private void generateTestData(HRegion region, int numRows) throws IOException {
