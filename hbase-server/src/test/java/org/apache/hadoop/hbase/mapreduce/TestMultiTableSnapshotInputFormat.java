...
public class TestMultiTableSnapshotInputFormat extends MultiTableInputFormatTestBase {
  ...
  public static void setUpSnapshots() throws Exception {
    ...
    for (String tableName : TABLES) {
      SnapshotTestingUtils.createSnapshotAndValidate(TEST_UTIL.getHBaseAdmin(),
        TableName.valueOf(tableName), ImmutableList.of(MultiTableInputFormatTestBase.INPUT_FAMILY),
        null, snapshotNameForTable(tableName), FSUtils.getRootDir(TEST_UTIL.getConfiguration()),
  ...
  public void setUp() throws Exception {
    this.restoreDir = new Path(TEST_UTIL.getDataTestDir(), UUID.randomUUID().toString());
  }
  ...
  protected Map<String, Collection<Scan>> getSnapshotScanMapping(final List<Scan> scans) {
    return Multimaps.index(scans, TestMultiTableSnapshotInputFormat::getScanTableName).asMap();
  }

  static String getScanTableName(Scan scan) {
    return snapshotNameForTable(
      Bytes.toStringBinary(scan.getAttribute(Scan.SCAN_ATTRIBUTES_TABLE_NAME)));
  }

  static String snapshotNameForTable(String tableName) {
