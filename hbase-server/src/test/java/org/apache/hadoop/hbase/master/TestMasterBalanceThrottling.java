...
public class TestMasterBalanceThrottling {
  ...
  public void testThrottlingByMaxRitPercent() throws Exception {
    // Set max balancing time to 500 ms and max percent of regions in transition to 0.03
    TEST_UTIL.getConfiguration().setInt(HConstants.HBASE_BALANCER_MAX_BALANCING, 500);
    TEST_UTIL.getConfiguration().setDouble(HConstants.HBASE_MASTER_BALANCER_MAX_RIT_PERCENT, 0.03);
    TEST_UTIL.startMiniCluster(2);
    ...
    checker.interrupt();
    checker.join();
    // The max number of regions in transition is 100 * 0.03 = 3
    assertTrue("max regions in transition: " + maxCount.get(), maxCount.get() == 3);
