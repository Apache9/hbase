...
public class TestAsyncTableScanMetrics {
  ...
  public static void setUp() throws Exception {
    ...
    try (HTable table = UTIL.createTable(TABLE_NAME, CF)) {
      NUM_REGIONS = UTIL.createMultiRegions(table, CF);
      table.put(Arrays.asList(new Put(Bytes.toBytes("zzz1")).add(CF, CQ, VALUE),
        new Put(Bytes.toBytes("zzz2")).add(CF, CQ, VALUE),
        new Put(Bytes.toBytes("zzz3")).add(CF, CQ, VALUE)));
    }
    CONN = HConnectionManager.createAsyncConnection(UTIL.getConfiguration()).get();
  }
  ...
  public void testScanMetrics() throws Exception {
    ...
    long bytes = results.stream().flatMap(r -> Arrays.asList(r.rawCells()).stream())
        .mapToLong(c -> CellUtil.estimatedSizeOf(c)).sum();
