...
public class TestScannerHeartbeatMessages {
  ...
  public void testImportanceOfHeartbeats(Callable<Void> testCallable) throws InterruptedException {
    ...
    HeartbeatHRegionServer.heartbeatsEnabled = false;
    try {
      testCallable.call();
    } catch (Exception e) {
      return;
  ...
  /**
   * Custom HRegionServer instance that instantiates {@link HeartbeatHRegionServer} in place of
   * {@link HeartbeatHRegionServer} to allow us to toggle support for heartbeat messages
   */
  ...
  private static class HeartbeatHRegion extends HRegion {
    ...
    public HeartbeatHRegion(HRegionFileSystem fs, HLog log, Configuration confParam,
  ...
  private static class HeartbeatRegionScanner extends RegionScannerImpl {
    ...
    protected void initializeKVHeap(List<KeyValueScanner> scanners,
        List<KeyValueScanner> joinedScanners, HRegion region) throws IOException {
      this.storeHeap = new HeartbeatKVHeap(scanners,
          new KeyValueHeap.KVScannerComparator(region.getComparator()));
      if (!joinedScanners.isEmpty()) {
        this.joinedHeap = new HeartbeatKVHeap(joinedScanners,
            new KeyValueHeap.KVScannerComparator(region.getComparator()));
      }
    }
  }
