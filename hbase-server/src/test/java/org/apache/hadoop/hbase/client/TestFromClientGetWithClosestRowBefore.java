  ...
  public void testGetWithClosestRowBeforeWhenSplitRegion() throws Exception {
    ...
    try {
      for (int i = 0; i < 1000; i++) {
        ...
        Put put = new Put(data).add(COLUMN_FAMILY, null, data);
        table.put(put);
      }
    } finally {
      table.close();
    ...
    table = new HTable(CONF, TableName.META_TABLE_NAME);
    try {
      t.start();
  ...
  public void testClosestRowIsLatestPutRow() throws IOException {
    ...
    Thread t = new Thread() {
      public void run() {
        try {
          ...
          HTable table = new HTable(CONF, TEST_TABLE.getName());
          try {
            for (int i = initialRowkeys[0] + 1; i < initialRowkeys[1]; i++) {
              byte[] data = Bytes.toBytes(String.format("%026d", i));
              Put put = new Put(data).add(COLUMN_FAMILY, null, value);
              table.put(put);
            }
          } catch (Exception e) {
            table.close();
    ...
    try {
      ...
      for (int i = 0; i < initialRowkeys.length; i++) {
        ...
        Put put = new Put(rowKey).add(COLUMN_FAMILY, null, rowKey);
        table.put(put);
      }
