...
import static org.apache.hadoop.hbase.quotas.ThrottleQuotaTestUtil.doAppends;
...
public class TestQuotaThrottle {
  ...
  public void testTableAppend() throws Exception {
    ...
    admin.setQuota(QuotaSettingsFactory.throttleTable(TABLE_NAMES[0],
      ThrottleType.READ_CAPACITY_UNIT, 6, TimeUnit.MINUTES));
    admin.setQuota(QuotaSettingsFactory.throttleTable(TABLE_NAMES[0],
      ThrottleType.WRITE_CAPACITY_UNIT, 8, TimeUnit.MINUTES));
    triggerTableCacheRefresh(TEST_UTIL, false, TABLE_NAMES[0]);

    // should execute at max 6 capacity units because each get size is 1 capacity unit
    assertEquals(6, doAppends(20, 1, FAMILY, QUALIFIER, tables[0]));

    admin.setQuota(QuotaSettingsFactory.throttleTable(TABLE_NAMES[0],
      ThrottleType.READ_CAPACITY_UNIT, 10, TimeUnit.MINUTES));
    triggerTableCacheRefresh(TEST_UTIL, false, TABLE_NAMES[0]);
    waitMinuteQuota();
    assertEquals(8, doAppends(20, 1, FAMILY, QUALIFIER, tables[0]));
