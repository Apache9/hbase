...
public class TestSmallReversedScanner {
  ...
  private static final byte[] TABLE_NAME = Bytes.toBytes("testReversedSmall");
  ...
  public static void setUpBeforeClass() throws Exception {
    ...
    htable = TEST_UTIL.createTable(TableName.valueOf(TABLE_NAME), COLUMN_FAMILY, splitKeys);
  }
  ...
  public void tearDown() throws IOException {
    TEST_UTIL.truncateTable(TableName.valueOf(TABLE_NAME));
  }

  @Test
  public void testReversedSmallScan01() throws IOException {
    ...
    for (int caseIndex = 0; caseIndex < keysCases.length; caseIndex++) {
      testReversedSmallScanInternal(keysCases[caseIndex]);
      TEST_UTIL.truncateTable(TABLE_NAME);
    }
  }

  private void testReversedSmallScanInternal(String[] inputRowKeys) throws IOException {
    int rowCount = inputRowKeys.length;

    for (int i = 0; i < rowCount; i++) {
      Put put = new Put(Bytes.toBytes(inputRowKeys[i]));
      put.add(COLUMN_FAMILY, null, Bytes.toBytes(i));
      htable.put(put);
    }
   ...
   *  current HBase client reversed small scanner will return infinity results with RowKey={0x00}.
   * @throws IOException
   */
  @Test
  public void testReversedSmallScan02() throws IOException {
    Put put = new Put(new byte[]{(char) 0x00});
    put.add(COLUMN_FAMILY, null, Bytes.toBytes(0));
    htable.put(put);
