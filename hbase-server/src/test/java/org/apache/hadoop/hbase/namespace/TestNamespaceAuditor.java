...
public class TestNamespaceAuditor {
  protected static final Log LOG = LogFactory.getLog(TestNamespaceAuditor.class);
  ...
  public void testTableOperations() throws Exception {
    ...
    tableDescOne.addFamily(new HColumnDescriptor(HConstants.CATALOG_FAMILY_STR));
    HTableDescriptor tableDescTwo =
        new HTableDescriptor(TableName.valueOf(nsp + TableName.NAMESPACE_DELIM + "table2"));
    tableDescTwo.addFamily(new HColumnDescriptor(HConstants.CATALOG_FAMILY_STR));
    HTableDescriptor tableDescThree =
        new HTableDescriptor(TableName.valueOf(nsp + TableName.NAMESPACE_DELIM + "table3"));
    tableDescThree.addFamily(new HColumnDescriptor(HConstants.CATALOG_FAMILY_STR));
  ...
  public void testDeleteTable() throws Exception {
    ...
    tableDescOne.addFamily(new HColumnDescriptor(HConstants.CATALOG_FAMILY));
    HTableDescriptor tableDescTwo =
        new HTableDescriptor(TableName.valueOf(namespace + TableName.NAMESPACE_DELIM + "table2"));
    tableDescTwo.addFamily(new HColumnDescriptor(HConstants.CATALOG_FAMILY));
    ADMIN.createTable(tableDescOne);
    ADMIN.createTable(tableDescTwo, Bytes.toBytes("AAA"), Bytes.toBytes("ZZZ"), 5);
  ...
  public void testRegionMerge() throws Exception {
    ...
    NamespaceDescriptor nspDesc =
        NamespaceDescriptor.create(nsp1)
            ...
            .addConfiguration(TableNamespaceManager.KEY_MAX_TABLES, "2").build();
    ADMIN.createNamespace(nspDesc);
    final TableName tableOne = TableName.valueOf(nsp1 + TableName.NAMESPACE_DELIM + "table2");
    ...
    HTable htable = new HTable(UTIL.getConfiguration(), tableOne);
    UTIL.loadNumericRows(htable, Bytes.toBytes("info"), 1000, 1999);
    
    ADMIN.flush(tableOne.getNameAsString());
    ...
    ADMIN.mergeRegions(hris.get(0).getEncodedNameAsBytes(), hris.get(1).getEncodedNameAsBytes(),
      false);
    waitForMergeToComplete(tableOne, encodedRegionNamesToMerge);
    hris = ADMIN.getTableRegions(tableOne);
    assertEquals(initialRegions - 1, hris.size());
    Collections.sort(hris);

    final HRegionInfo hriToSplit = hris.get(1);
    ADMIN.split(tableOne.getName(), Bytes.toBytes("500"));
    ...
    UTIL.waitFor(10000, 100, new Predicate<Exception>() {
      ...
      public boolean evaluate() throws Exception {
        ...
        for (HRegionInfo hri : ADMIN.getTableRegions(tableOne)) {
    ...
    ADMIN.split(tableOne.getName(),
      TableInputFormatBase.getSplitKey(hriToSplit2.getStartKey(), hriToSplit2.getEndKey(), true));
    waitForMergeToComplete(tableOne, encodedRegionNamesToMerge);
    assertEquals(initialRegions, ADMIN.getTableRegions(tableOne).size());
  }
  ...
  public void testRegionOperations() throws Exception {
    ...
    try {
      ...
      assertTrue(exp instanceof QuotaExceededException);
      LOG.info(exp);
      constraintViolated = true;
    } finally {
    ...
    stateInfo = getNamespaceState(nsp1);
    assertEquals(1, stateInfo.getTables().size());
    assertEquals(2, stateInfo.getRegionCount());
    actualRegion = UTIL.getHBaseCluster().getRegions(tableOne).get(0);
    observer =
        (CustomObserver) actualRegion.getCoprocessorHost().findCoprocessor(
          CustomObserver.class.getName());
    assertNotNull(observer);
    ADMIN.majorCompact(actualRegion.getRegionName());
    while (ADMIN.getCompactionState(actualRegion.getRegionName()) != CompactionState.NONE) {
      Thread.sleep(1000);
    }
    ADMIN.split(
      tableOne.getName(),
  ...
  public void testStatePreserve() throws Exception {
    ...
    tableDescThree.addFamily(new HColumnDescriptor(HConstants.CATALOG_FAMILY));
  ...
  private void deleteTable(final TableName tableName) throws Exception {
    ...
    MasterSyncObserver observer =
        (MasterSyncObserver) UTIL.getHBaseCluster().getMaster().getCoprocessorHost()
  ...
  public void testCloneSnapshot() throws Exception {
    ...
    int tableLength = MetaScanner.allTableRegions(ADMIN.getConnection().getConfiguration(),
      ADMIN.getConnection(), tableName, true).size();
    assertEquals(tableName.getNameAsString() + " should have four regions.", 4, tableLength);

    tableLength = MetaScanner.allTableRegions(ADMIN.getConnection().getConfiguration(),
      ADMIN.getConnection(), tableName, true).size();
  ...
  public void testRestoreSnapshot() throws Exception {
    ...
    ADMIN.split(tableName1.getName(), Bytes.toBytes("JJJ"));
    Thread.sleep(2000);
