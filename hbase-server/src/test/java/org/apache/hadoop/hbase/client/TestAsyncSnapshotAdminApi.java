...
public class TestAsyncSnapshotAdminApi extends TestAsyncAdminBase {
  ...
  public void testTakeSnapshot() throws Exception {
    HBaseAdmin syncAdmin = TEST_UTIL.getHBaseAdmin();

    HTable table = TEST_UTIL.createTable(tableName, Bytes.toBytes("f1"));
    for (int i = 0; i < 3000; i++) {
      table.put(new Put(Bytes.toBytes(i)).add(Bytes.toBytes("f1"), Bytes.toBytes("cq"),
        Bytes.toBytes(i)));
    }

    admin.snapshot(snapshotName1, tableName).get();
    admin.snapshot(snapshotName2, tableName).get();
    List<SnapshotDescription> snapshots = syncAdmin.listSnapshots().stream()
        .map(ProtobufUtil::createSnapshotDesc).collect(Collectors.toList());
  ...
  private void assertResult(HTable table, int expectedRowCount) throws IOException {
    Scan scan = new Scan();
