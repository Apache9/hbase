...
public class TestCacheOnWriteInSchema {
  ...
  public void testCacheOnWriteInSchema() throws IOException {
    ...
    StoreFileWriter writer = store.getStoreEngine().getStoreFileTracker()
      .createWriter(CreateStoreFileWriterParams.create().setMaxKeyCount(Integer.MAX_VALUE)
        .setCompression(HFile.DEFAULT_COMPRESSION_ALGORITHM).setCompaction(false)
        .setIncludeMVCCReadpoint(true).setIncludesTag(false).setShouldDropBehind(false));
