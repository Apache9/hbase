...
public class TestVerifySnapshot {
  ...
  public void setUp() throws Exception {
    ...
    HTableDescriptor table2 = new HTableDescriptor(tableName2);
    table2.addFamily(fam);

    utility1.getHBaseAdmin().createTable(table);
    utility1.getHBaseAdmin().createTable(table2);

    htable1 = new HTable(utility1.getConfiguration(), tableName);
    htable2 = new HTable(utility1.getConfiguration(), tableName2);
  }
  ...
  private void testSmallBatch() throws Exception {
    ...
    for (int i = 0; i < NB_ROWS_IN_BATCH; i++) {
      ...
      put.add(famName, row, System.currentTimeMillis(), row);
      htable1.put(put);
      htable2.put(put);
    }
  ...
  public void testVerifySnapshot() throws Exception {
    ...
    String[] args = new String[] { "--snapshot-URI", sourceSnapshotName, "--compare-snapshot-URI",
        ...
        "--compare-restoreDir", temPath1, "--compare-cluster-hdfs", peerFSAddress,
        "--compare-cluster-hbase-root", FSUtils.getRootDir(conf1).toString() };
