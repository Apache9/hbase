...
public class TestHBaseStreamCopyTableToTalos {
  ...
  private void deleteData(HTable t, byte[] family, byte[] column) throws IOException {
    Delete d1 = new Delete(rowkeys[0]);
    t.delete(d1);
    Delete d2 = new Delete(rowkeys[1]).deleteFamily(family);
    t.delete(d2);
    Delete d3 = new Delete((rowkeys[2])).deleteColumns(family, column);
    t.delete(d3);
  }
}
