...
public class TestOpenSeqNumUnexpectedIncrease {
  ...
  public void test() throws IOException, InterruptedException {
    ...
    while (UTIL.getMiniHBaseCluster().getMaster().getServerManager().getOnlineServersList().size() < 3) {
      Thread.sleep(1000);
      System.err.println("not enough online servers");
    }
    ...
    UTIL.getAdmin().move(region1.getRegionInfo().getEncodedNameAsBytes(),
      Bytes.toBytes(src.getServerName().getServerName()));

    HRegion region2 = UTIL.getMiniHBaseCluster().getRegions(TABLE_NAME).get(0);

    long openSeqNum2 = region2.getOpenSeqNum();

    System.out.println(region.hashCode() + ": " + openSeqNum);
    System.out.println(region1.hashCode() + ": " + openSeqNum1);
    System.out.println(region2.hashCode() + ": " + openSeqNum2);
  }
}
