...
public class TestCompactionWithThroughputController {
  ...
  private long testCompactionLimit() throws Exception {
    ...
    conf.setLong(
      DefaultCompactionThroughputController.HBASE_HSTORE_COMPACTION_MAX_THROUGHPUT_HIGHER_BOUND,
      1024L * 1024);
    conf.setLong(
      DefaultCompactionThroughputController.HBASE_HSTORE_COMPACTION_MAX_THROUGHPUT_LOWER_BOUND,
      1024L * 1024);
    conf.set(CompactionThroughputControllerFactory.HBASE_THROUGHPUT_CONTROLLER_KEY,
      DefaultCompactionThroughputController.class.getName());
    ...
    try {
      ...
      assertTrue((double) store.getStorefilesSize() / duration * 1000 < 1024L * 1024 * 1.2);
  ...
  private long testCompactionNoLimit() throws Exception {
    Configuration conf = TEST_UTIL.getConfiguration();
    conf.set(StoreEngine.STORE_ENGINE_CLASS_KEY, DefaultStoreEngine.class.getName());
    conf.setInt(CompactionConfiguration.MIN_KEY, 100);
    conf.setInt(CompactionConfiguration.MAX_KEY, 200);
  ...
  public void testCompaction() throws Exception {
    long limitTime = testCompactionLimit();
    long noLimitTime = testCompactionNoLimit();
  ...
  public void testThroughputTuning() throws Exception {
    ...
    conf.set(CompactionThroughputControllerFactory.HBASE_THROUGHPUT_CONTROLLER_KEY,
      DefaultCompactionThroughputController.class.getName());
    conf.setInt(
      DefaultCompactionThroughputController.HBASE_HSTORE_COMPACTION_THROUGHPUT_TUNE_PERIOD, 1000);
    TEST_UTIL.startMiniCluster(1);
    ...
    try {
      ...
      DefaultCompactionThroughputController throughputController =
          (DefaultCompactionThroughputController) regionServer.compactSplitThread
  ...
  @Test
  public void testGetNormalizedStorefilesCount() throws Exception {
    Configuration conf = TEST_UTIL.getConfiguration();
