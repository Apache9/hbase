...
public class TableSchemaModel implements Serializable, ProtobufMessageHandler {
  ...
  private static final QName COMPRESSION =
    new QName(ColumnFamilyDescriptorBuilder.COMPRESSION);
   ...
   */
  public TableSchemaModel(TableDescriptor htd) {
  ...
  public TableDescriptor getTableDescriptor() {
    TableDescriptorBuilder builder = TableDescriptorBuilder.newBuilder(TableName.valueOf(getName()));
    for (Map.Entry<QName, Object> e: getAny().entrySet()) {
      builder.setValue(e.getKey().getLocalPart(), e.getValue().toString());
    }
    for (ColumnSchemaModel column: getColumns()) {
      ...
      for (Map.Entry<QName, Object> e: column.getAny().entrySet()) {
        colBuilder.setValue(e.getKey().getLocalPart(), e.getValue().toString());
      }
